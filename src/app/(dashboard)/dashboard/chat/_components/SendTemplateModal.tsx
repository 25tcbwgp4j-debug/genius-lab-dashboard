...
export function SendTemplateModal({
  ...
  async function handleSend() {
    ...
    try {
      ...
      if (!res.ok) setError("Invio fallito. Verifica stato template su Meta Business.");
      else {
        onSent();
        onClose();
      }
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-4">
          ...
          {!loading && (
            <>
              <div>
                <label className="text-sm font-medium text-gray-700 mb-1 block">Seleziona</label>
                <select
                  value={selected}
                  onChange={(e) => {
                ...
                {tpl && <p className="text-xs text-gray-500 mt-1">{tpl.description}</p>}
              </div>

              {tpl && tpl.params.length > 0 && (
                <div className="space-y-2">
                  <div className="text-sm font-medium text-gray-700">Variabili</div>
                  {tpl.params.map((p, i) => (
                    <div key={p}>
